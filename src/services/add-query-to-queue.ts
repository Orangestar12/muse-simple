...
export default class AddQueryToQueue {
  ...
  public async addToQueue({
    ...
    if (newSongs.length === 1) {
      await interaction.editReply(`▶️ (**${firstSong.title}** added to the${addToFrontOfQueue ? ' front of the' : ''} queue${extraMsg})`);
    } else {
      await interaction.editReply(`▶️ (**${firstSong.title}** and ${newSongs.length - 1} other songs were added to the queue${extraMsg})`);
    }
  }
}
